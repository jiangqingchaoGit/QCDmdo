...
+ (void)showUnlockViewWithType:(YWUnlockViewType)type callBack:(CallBackBlock)callBack{
    ...
    unlockView.leftButton = [[UIButton alloc] initWithFrame:CGRectMake(KSCALE_WIDTH(0),KStatusHight , 44, 44)];
    [unlockView.leftButton setImage:[UIImage imageNamed:@"back_s"] forState:UIControlStateNormal];
    [unlockView.leftButton addTarget:unlockView action:@selector(leftAction:) forControlEvents:UIControlEventTouchUpInside];
    [unlockView addSubview:unlockView.leftButton];
    if (unlockView.type == YWUnlockViewCreate) {
        unlockView.leftButton.hidden = NO;
    }else{
        unlockView.leftButton.hidden = YES;

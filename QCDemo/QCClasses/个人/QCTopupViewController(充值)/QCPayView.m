...
- (void)initUI {
    ...
    self.interestLabel  = [[UILabel alloc] initWithFrame:CGRectMake(KSCALE_WIDTH(0), KSCALE_WIDTH(140), KSCALE_WIDTH(315), KSCALE_WIDTH(30))];
    if ([self.type isEqualToString:@"3"]) {
        self.interestLabel.text = [NSString stringWithFormat:@"服务费：¥%.2f (0.2%%)",[self.messageDic[@"red_price"] floatValue] * 0.02];
    }
    self.interestLabel.font = K_14_FONT;
    self.interestLabel.textColor = [QCClassFunction stringTOColor:@"#000000r"];
    ...
    if (sender.text.length == 6) {
        ...
        [QCAFNetWorking QCPOST:@"/api/user/verify_pwd" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
            ...
            if ([responseObject[@"msg"] isEqualToString:@"成功"]) {
                ...
                if ([self.type isEqualToString:@"3"]) {
                    [self drawalWithKey:responseObject[@"data"][@"key"]];
                }
                if ([self.type isEqualToString:@"4"]) {
                    //  充值
                }
                ...
                if ([self.type isEqualToString:@"5"]) {
                    [self payWithKey:responseObject[@"data"][@"key"]];
                }
...
#pragma mark - GETDATA
- (void)drawalWithKey:(NSString *)keyStr {
    //  提现
        NSString * str = [NSString stringWithFormat:@"amount=%@&key=%@&my_bank_id=%@&token=%@&uid=%@",self.messageDic[@"red_price"],keyStr,self.messageDic[@"method"],K_TOKEN,K_UID?K_UID:@""];
        
        NSString * signStr = [QCClassFunction MD5:str];
    NSDictionary * dic = @{@"amount":self.messageDic[@"red_price"],@"key":keyStr,@"my_bank_id":self.messageDic[@"method"],@"token":K_TOKEN,@"uid":K_UID?K_UID:@""};
        
        NSString * jsonString = [QCClassFunction jsonStringWithDictionary:dic];
        NSString * outPut = [[QCClassFunction AES128_Encrypt:K_AESKEY encryptData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]] base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
        
        NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
        [QCAFNetWorking QCPOST:@"/api/finance/withdraw" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
            ...
            if ([responseObject[@"status"] intValue] == 1) {
                ...
                [[QCClassFunction parentController:self].navigationController pushViewController:withdrawalListViewController animated:YES];
                
                //  到提现界面
            }else{
                [QCClassFunction showMessage:responseObject[@"msg"] toView:[QCClassFunction getCurrentViewController].view];
...
- (void)payWithKey:(NSString *)keyStr {
    //  支付
        NSString * str = [NSString stringWithFormat:@"address=%@&gid=%@&key=%@&method=%@&num=1&remark=&token=%@&uid=%@&usermobile=%@&username=%@",self.messageDic[@"address"],self.messageDic[@"gid"],keyStr,self.messageDic[@"method"],K_TOKEN,K_UID?K_UID:@"",self.messageDic[@"usermobile"],self.messageDic[@"username"]];
        
        NSString * signStr = [QCClassFunction MD5:str];
    NSDictionary * dic = @{@"address":self.messageDic[@"address"],@"gid":self.messageDic[@"gid"],@"key":keyStr,@"method":self.messageDic[@"method"],@"num":@"1",@"remark":@"",@"token":K_TOKEN,@"uid":K_UID?K_UID:@"",@"usermobile":self.messageDic[@"usermobile"],@"username":self.messageDic[@"username"]};
        
        NSString * jsonString = [QCClassFunction jsonStringWithDictionary:dic];
        NSString * outPut = [[QCClassFunction AES128_Encrypt:K_AESKEY encryptData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]] base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
        
        NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
        [QCAFNetWorking QCPOST:@"/api/order/payOrder" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {

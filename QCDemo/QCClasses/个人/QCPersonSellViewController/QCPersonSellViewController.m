...
@interface QCPersonSellViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation QCPersonSellViewController
...
- (void)GETDATA {
    ...
    [QCAFNetWorking QCPOST:@"/api/order/sellorder" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
...
- (void)refundAction:(UIButton *)sender {
    //  退款
    QCPersonSellCell * cell = (QCPersonSellCell *)[[sender superview] superview];
    ...
    NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
    [QCAFNetWorking QCPOST:@"/api/order/assentrefund" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
...
- (void)reminderAction:(UIButton *)sender {
    ...
    NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
    [QCAFNetWorking QCPOST:@"/api/order/refusalrefund" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
...
- (void)goodsAction:(UIButton *)sender {
    ...
    QCDeliveryViewController * deliveryViewController = [[QCDeliveryViewController alloc] init];
    deliveryViewController.hidesBottomBarWhenPushed = YES;
    deliveryViewController.model = model;
...
//    self.tableView.backgroundColor = [QCClassFunction stringTOColor:@"#F2F2F2"];
    ...
    [self.tableView registerClass:[QCPersonSellCell class] forCellReuseIdentifier:@"cell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QCPersonSellCell * cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];

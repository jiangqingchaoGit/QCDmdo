...
@interface QCGroupChooseViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation QCGroupChooseViewController
...
- (void)addPeopleDATA {
    ...
    NSString * fuid = [NSString stringWithFormat:@"%@,",[self.peopleArr componentsJoinedByString:@","]];

    //  添加群聊
    NSString * str = [NSString stringWithFormat:@"fuid=%@&group_id=%@&token=%@&uid=%@",fuid,self.group_id,K_TOKEN,K_UID];
    NSString * signStr = [QCClassFunction MD5:str];
    NSDictionary * dic = @{@"fuid":fuid,@"group_id":self.group_id,@"token":K_TOKEN,@"uid":K_UID};
    NSString * jsonString = [QCClassFunction jsonStringWithDictionary:dic];
    NSString * outPut = [[QCClassFunction AES128_Encrypt:K_AESKEY encryptData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]] base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
    NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
    ...
    [QCAFNetWorking QCPOST:@"/api/chat/group_apply" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.searchStatus == 0) {
        ...
        if ([self.nameArr containsObject:model.nick]) {

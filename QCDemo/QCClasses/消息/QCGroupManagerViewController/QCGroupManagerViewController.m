//
//  QCGroupManagerViewController.m
//  QCDemo
//
...
@interface QCGroupManagerViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation QCGroupManagerViewController
...
- (void)ADDManager {
    ...
    //  移除群聊
    NSString * str = [NSString stringWithFormat:@"group_id=%@&muid=%@&token=%@&uid=%@&user_type=%@",self.group_id,out_uid,K_TOKEN,K_UID,@"2"];
    NSString * signStr = [QCClassFunction MD5:str];
    NSDictionary * dic = @{@"group_id":self.group_id,@"muid":out_uid,@"token":K_TOKEN,@"uid":K_UID,@"user_type":@"2"};
    NSString * jsonString = [QCClassFunction jsonStringWithDictionary:dic];
    NSString * outPut = [[QCClassFunction AES128_Encrypt:K_AESKEY encryptData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]] base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
    NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};

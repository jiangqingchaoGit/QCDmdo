//
//  QCNewGroupManagerViewController.m
//  QCDemo
//
...
@interface QCNewGroupManagerViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation QCNewGroupManagerViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)ADDManager {
    ...
    //  移除群聊
    NSString * str = [NSString stringWithFormat:@"fuid=%@&group_id=%@&token=%@&uid=%@",out_uid,self.group_id,K_TOKEN,K_UID];
    NSString * signStr = [QCClassFunction MD5:str];
    NSDictionary * dic = @{@"fuid":out_uid,@"group_id":self.group_id,@"token":K_TOKEN,@"uid":K_UID};
    NSString * jsonString = [QCClassFunction jsonStringWithDictionary:dic];
    NSString * outPut = [[QCClassFunction AES128_Encrypt:K_AESKEY encryptData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]] base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
    NSDictionary * dataDic = @{@"sign":signStr,@"data":outPut};
    ...
    [QCAFNetWorking QCPOST:@"/api/chat/transfer" parameters:dataDic success:^(NSURLSessionDataTask *operation, id responseObject) {

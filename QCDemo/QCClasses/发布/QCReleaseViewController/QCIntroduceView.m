...
@interface QCIntroduceView()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate>
...
@end
@implementation QCIntroduceView
...
- (void)deleteBtnClik:(UIButton *)sender {
    ...
    if ([self collectionView:self.collectionView numberOfItemsInSection:0] <= _selectedPhotos.count) {
        ...
        if (_selectedPhotos.count <= 3) {
            self.frame = CGRectMake(KSCALE_WIDTH(20), KSCALE_WIDTH(380), KSCALE_WIDTH(335), KSCALE_WIDTH(90));
            self.collectionView.frame = self.bounds;
            viewH = KSCALE_WIDTH(0);

        }else if (_selectedPhotos.count <= 7 && _selectedPhotos.count > 3) {
            
            self.frame = CGRectMake(KSCALE_WIDTH(20), KSCALE_WIDTH(380), KSCALE_WIDTH(335), KSCALE_WIDTH(172));
            self.collectionView.frame = self.bounds;
            viewH = KSCALE_WIDTH(82);

        }else{
            self.frame = CGRectMake(KSCALE_WIDTH(20), KSCALE_WIDTH(380), KSCALE_WIDTH(335), KSCALE_WIDTH(254));
            self.collectionView.frame = self.bounds;
            viewH = KSCALE_WIDTH(164);
;

        }
        
        [releaseViewController upSizeFrameWithHight:viewH];
